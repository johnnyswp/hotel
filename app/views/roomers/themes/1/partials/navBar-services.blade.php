...
<div class="col s12 m12 l12 nav">
	   	...
	   	<a class="waves-effect waves-light btn modal-trigger right btn-ico inicio" href="/roomer/seleccion">
		...
		<a class="waves-effect waves-light btn modal-trigger right btn-ico atras" href="{{$back}}">
...
<div id="modal4" class="modal grey lighten-4">
	<div class="modal-content ">
		<ul class="collection with-header grey lighten-4">
	        <li class="collection-header teal-text"><h4>{{$lang->txt_orders}}</h4></li>
	        @foreach($orders as $o)
	        	<?php $a++;?>
	        	<li class="collection-item  indigo-text"><a order="order_{{$o->id}}" href="#orderModal" idn="{{$a}}" class="showOrder modal-trigger">#: {{$lang->txt_order}}  {{$a}}</a></li>	        	
	        @endforeach
      </ul>
	</div>
</div>
...
<div id="modal1" class="modal bottom-sheet  grey lighten-4">
	...
	<div class="modal-content">
			...
			@if($lang->language_id==Session::get('lang_id'))
			<p>
			<input name="lang" type="radio" id="test{{$lang->language_id}}"  value="{{$lang->language_id}}" checked="checked" />
				<label class=" red-text accent-4" for="test{{$lang->language_id}}">{{Language::find($lang->language_id)->language}}</label>
			</p>
			@else
			<p>
				<input name="lang" type="radio" id="test{{$lang->language_id}}" value="{{$lang->language_id}}" />
				<label class=" red-text accent-4" for="test{{$lang->language_id}}">{{Language::find($lang->language_id)->language}}</label>
			</p>
			@endif
			@endforeach
	...
	<div class="modal-footer grey lighten-4">
		<button type="submit" class=" modal-action modal-close waves-effect waves-green btn-flat">{{$llang->txt_aceptar}}</button>
	</div>
	</form>
</div> 
...
<div id="modal_eliminar_pedido" class="modal">
    <div class="modal-content">
      <h4 class="teal-text flow-text center">{{$llang->txt_message_eliminar_pedido}}</h4>       
    </div>
    <div class="modal-footer">
      
      <a href="#" id="btn_cancel_order" class="modal-action btn   waves-effect waves-green  ">{{$llang->txt_si}}</a>

      <a href="#"  class=" modal-action btn red modal-close waves-effect waves-green  " style="margin-right: 15px; ">{{$llang->txt_no}}</a>
    </div>
  </div>
  ...
  <div id="modal_cancelar_pedido" class="modal">
    ...
    <div class="modal-footer">
      ...
      <a href="#" id="btn_cancel_order2" class="modal-action btn   waves-effect waves-green  ">{{$llang->txt_si}}</a>
  ...
  <div id="modal_no_horario" class="modal">
    <div class="modal-content">
      <h4 class="teal-text flow-text center">{{$llang->txt_message_no_horario}}</h4>       
    </div>
    <div class="modal-footer">
      <a href="#!" id="modal_no_horario_close" onclick="return window.location.reload();" style="width: 100%; "  class="modal-action btn   waves-effect waves-green  ">{{$llang->txt_aceptar}}</a> <br>
