	...
	<div class="row">
		...
		<section class="panel">
			<header class="panel-heading sm" data-color="theme-inverse">
				<h2><strong>{{trans('main.alta de')}}</strong>  {{trans('main.activity')}}</h2>
				@if (Session::has('flash_message'))
                <label class="color" style="color: white;background-color: rgba(0, 204, 23, 0.29);">{{ Session::get('flash_message') }}</label>
                @endif
			</header>
			<div class="panel-body">
				{{ Form::open(array('action' => 'HotelActivityController@store', 'files'=>true))}} 
					...
					@foreach($langs as $lang)
					    ...
					    <div class="form-group">
					    	<div>
					    		{{ Form::text('public_'.$lang->language->language, NULL, ['class' => 'form-control', 'placeholder'=>trans('main.public en').' '.$lang->language->language, 'autocomplete'=>'off']) }}
                                {{ errors_for('public_'.$lang->language->language, $errors) }}
					    	</div>
					    </div>
					    <div class="form-group">
					    	<div>
					    		{{ Form::text('zone_'.$lang->language->language, NULL, ['class' => 'form-control', 'placeholder'=>trans('main.zone en').' '.$lang->language->language, 'autocomplete'=>'off']) }}
					    ...
					    @else
					        ...
					        <div class="form-group">
					        	<div>
					        		{{ Form::text('zone_'.$lang->language->language, NULL, ['class' => 'form-control','placeholder'=>trans('main.zona en').' '.$lang->language->language, 'autocomplete'=>'off']) }}
                                    {{ errors_for('zone_'.$lang->language->language, $errors) }}
					        	</div>
					        </div>
					    @endif
