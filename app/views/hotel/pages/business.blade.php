	...
	<div class="row">
		...
		<section class="panel">
			<header class="panel-heading sm" data-color="theme-inverse">
				<h2><strong>{{trans('main.business')}}</strong></h2>
			...
			<div class="panel-body">
				<div class="form-group">
					<div>
						<a href="{{url('hotel/business/create')}}" class="btn btn-theme">{{trans('main.nuevo')}} {{trans('main.business')}}</a>
					</div>
				</div>

        {{ Form::open(array('action' => 'HotelBusinessController@index', 'files'=>true, 'id'=>'form-service', 'method'=>'GET'))}}
					<div class="form-group">
             <label class="control-label">{{trans('main.service')}}</label>
             <div>
                 {{ Form::select('service', $services, $first_service, ['id'=>'service','class' => 'form-control selectpicker', 'data-size'=>'10', 'data-live-search'=>'true', 'autocomplete'=>'off']) }}
                 {{ errors_for('service', $errors) }}
             </div>
         </div>
				{{ Form::close() }}
				...
				<div class="panel panel-default">
			        ...
			        <ul id="sortable" class="col-md-12 list">
			          	...
			          	<?php 
                             $businessLang = BusinessLang::where('business_id', $item->id)->where('language_id', $lang->language_id)->first();
				  	    ?>
			          	<li id="listItem_{{$item->id}}" class="col-md-12 item-category" style="padding-bottom: 5px; padding-top: 5px;">
			          		...
			          		<div class="col-md-5" style="text-align: center;">{{$businessLang->name}}</div>
			          		<div class="col-md-2" >
			          		...
			          		<div class="col-md-1">
			          		    <a href="{{url('hotel/business/'.$item->id.'/edit')}}" type="button" class="btn btn-md btn-primary"><span class="glyphicon glyphicon-pencil" aria-hidden="true" style="color: white;"></span></a>
			          		</div>
			          		<div class="col-md-1">
                                ...
                                <div id="form_notice_{{$item->id}}" style="display: none;">
                                  {{ Form::open(array('class'=>'pf-form pform_custom','url' => 'hotel/business/'.$item->id)) }}
