	...
	<div class="row">
		...
		<section class="panel">
			...
			<div class="panel-body">
				{{ Form::model($category, ['files' => 'true', 'method' => 'PATCH', 'route' => ['hotel.business.category.update', $category->id]]) }}
					...
					@foreach($langs as $lang_)
					    <?php
                            $categoryLang = CategoryLang::where('category_id', $category->id)->where('language_id', $lang_->language_id)->first();
					    ?>
					    @if($categoryLang)
					        @if($lang_->main==1)
