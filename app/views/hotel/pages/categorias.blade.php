	...
	<div class="row">
		...
		<section class="panel">
			<header class="panel-heading sm" data-color="theme-inverse">
				<h2><strong>{{trans('main.categories')}}</strong></h2>
			...
			<div class="panel-body" style="padding-left: 62px; padding-right: 128px;">
				<form class="form-horizontal" data-collabel="3" data-alignlabel="left"  data-label="color">
					...
					<div class="form-group">
						<div>
							<a href="{{url('hotel/category/create')}}" class="btn btn-theme">{{trans('main.nueva')}} {{trans('main.category')}}</a>
						</div>
					</div>
				</form>
				...
				<div class="panel panel-default">
			        ...
			        <ul id="sortable" class="col-md-12 list">
			          	...
			          	<li id="listItem_{{$category->id}}" class="col-md-12 item-category" style="padding-bottom: 5px; padding-top: 5px;">
			          		...
			          		<div class="col-md-5" style="text-align: center;">{{$nameCategory->name}}</div>
			          		<div class="col-md-2" >
			          			<div class="row" style="display: block; width: 75px; margin: 0 auto;     height: 25px;">
									<div class="col-sm-4 iSwitch flat-switch">
										<div class="switch">
											<input name="state" class="state" value="{{$category->id}}" type="checkbox" @if($category->state==1) checked @endif @if(Category_menu::state($category->id)!=true) disabled @endif>
										</div>
									</div>
								</div>
			          		</div>
			          		<div class="col-md-1">
			          		    <a href="{{url('hotel/category/'.$category->id.'/edit')}}" type="button" class="btn btn-md btn-primary"><span class="glyphicon glyphicon-pencil" aria-hidden="true" style="color: white;"></span></a>
			          		</div>
			          		<div class="col-md-1">
                      ...
                      <div id="form_notice_{{$category->id}}" style="display: none;">
                        {{ Form::open(array('class'=>'pf-form pform_custom','url' => 'hotel/category/'.$category->id)) }}
                          {{ Form::hidden("_method", "DELETE") }}
                          <div class="pf-element pf-heading">
                            <h3>{{trans('main.confirm delete category')}}</h3>
                            <p></p>
                          </div>
                        ...
                        {{ Form::close() }}
			          		...
			          		<div class="col-md-1">
			          		    <img src="@if(Category_menu::state($category->id)==true) {{url('assets/img/active.png')}} @else {{url('assets/img/no-active.png')}} @endif" width="35px" height="35px" style="display: block; margin: 0 auto;">
			          		</div>
			          	</li>
			          	@endforeach					
			          </ul>
...
$(function() {
    ...
    $("#sortable").sortable({
    	...
    	$.ajax({
            type: 'GET',
            url: "{{url('hotel/category-position')}}",
            data: order,
            success: function (data) {
                new PNotify({
                    title: "<p>Orden</p>",
