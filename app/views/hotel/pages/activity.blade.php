	...
	<div class="row">
		...
		<section class="panel">
			<header class="panel-heading sm" data-color="theme-inverse">
				<h2><strong>{{trans('main.activities')}}</strong></h2>
			...
			<div class="panel-body" style="padding-left: 62px; padding-right: 128px;">
				<form class="form-horizontal" data-collabel="3" data-alignlabel="left"  data-label="color">
					...
					<div class="form-group">
						<div>
							<a href="{{url('hotel/activity/create')}}" class="btn btn-theme">{{trans('main.nuevo')}} {{trans('main.activity')}}</a>
						</div>
					</div>
				</form>
				...
				<div class="panel panel-default">
			        ...
			        <ul id="sortable" class="col-md-12 list">
				  	    ...
				  	    ?>
			          	<li id="listItem_{{$activity->id}}" class="col-md-12 item-category" style="padding-bottom: 5px; padding-top: 5px;">
			          		<div class="col-md-2"><img src="{{$activity->picture}}" width="35px" height="35px"></div>
			          		<div class="col-md-5" style="text-align: center;">{{$activityLang->name}}</div>
			          		<div class="col-md-2" >
			          			<div class="row" style="display: block; width: 75px; margin: 0 auto;     height: 25px;">
									<div class="col-sm-4 iSwitch flat-switch">
										<div class="switch">
											<input name="state" class="state" value="{{$activity->id}}" type="checkbox" @if($activity->state==1) checked @endif @if(Activity::state($activity->id)!=true) disabled @endif>
										</div>
									</div>
								</div>
			          		</div>
			          		<div class="col-md-1">
			          		    <a href="{{url('hotel/activity/'.$activity->id.'/edit')}}" type="button" class="btn btn-md btn-primary"><span class="glyphicon glyphicon-pencil" aria-hidden="true" style="color: white;"></span></a>
			          		</div>
			          		<div class="col-md-1">
                      <button type="submit" class="btn btn-md btn-warning"  onclick="var notice = new PNotify({text: $('#form_notice_{{$activity->id}}').html(), icon: false, width: 'auto', hide: false, addclass: 'custom', icon: 'picon picon-32 picon-edit-delete', opacity: .8, nonblock: {nonblock: true }, animation: {effect_in: 'show', effect_out: 'slide'}, buttons: {closer: false, sticker: false }, insert_brs: false }); notice.get().find('form.pf-form').on('click', '[name=cancel]', function(){notice.remove(); }).submit(function(){$('#form_notice').submit(); });"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span></button>
                      <div id="form_notice_{{$activity->id}}" style="display: none;">
                        {{ Form::open(array('class'=>'pf-form pform_custom','url' => 'hotel/activity/'.$activity->id)) }}
                          {{ Form::hidden("_method", "DELETE") }}
                          <div class="pf-element pf-heading">
                            <h3>{{trans('main.confirm delete activity')}}</h3>
                            <p></p>
                          </div>
                        ...
                        {{ Form::close() }}
			          		...
			          		<div class="col-md-1">
			          		    <img src="@if(Activity::state($activity->id)==true) {{url('assets/img/active.png')}} @else {{url('assets/img/no-active.png')}} @endif" width="35px" height="35px" style="display: block; margin: 0 auto;">
			          		</div>
			          	</li>
			          	@endforeach					
			          </ul>
...
$(function() {
    ...
    $("#sortable").sortable({
    	...
    	$.ajax({
            type: 'GET',
            url: "{{url('hotel/activity-position')}}",
            data: order,
            success: function (data) {
                new PNotify({
                    title: "<p>Orden</p>",
