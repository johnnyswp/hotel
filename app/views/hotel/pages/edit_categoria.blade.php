	...
	<div class="row">
		...
		<section class="panel">
			<header class="panel-heading sm" data-color="theme-inverse">
				<h2><strong> {{trans('main.Editar')}}</strong>  {{trans('main.category')}}</h2>
				@if (Session::has('flash_message'))
                <label class="color" style="color: white;background-color: rgba(0, 204, 23, 0.29);">{{ Session::get('flash_message') }}</label>
                @endif
			</header>
			<div class="panel-body">
				{{ Form::model($category, ['files' => 'true', 'method' => 'PATCH', 'route' => ['hotel.category.update', $category->id]]) }}
					...
					@foreach($lang_active as $lang_)
					    ...
					    @if($name_category)
					        ...
					        <div class="form-group">
					        	<div>
					        		{{ Form::text($lang_->language->language, $name_category->name, ['class' => 'form-control', 'placeholder'=>$lang_->language->language, 'autocomplete'=>'off', 'required'=>'required']) }}
					        ...
					        @else
					            ...
					            <div class="form-group">
					            	<div>
					            		{{ Form::text($lang_->language->language, $name_category->name, ['class' => 'form-control', 'placeholder'=>$lang_->language->language, 'autocomplete'=>'off']) }}
                                        {{ errors_for($lang_->language->language, $errors) }}
					            	</div>
					            </div>
					        @endif
