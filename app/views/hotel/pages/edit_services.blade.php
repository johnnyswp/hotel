	...
	<div class="row">
		...
		<section class="panel">
			...
			<div class="panel-body">
				{{ Form::model($service, ['files' => 'true', 'method' => 'PATCH', 'route' => ['hotel.services.update', $service->id]]) }}
					...
					@foreach($lang_active as $lang_)
					    ...
					    @if($serviceLang)
					        ...
					        <div class="form-group">
					        	<div>
					        		{{ Form::text($lang_->language->language, $serviceLang->name, ['class' => 'form-control', 'placeholder'=>$lang_->language->language, 'autocomplete'=>'off', 'required'=>'required']) }}
                                    {{ errors_for($lang_->language->language, $errors) }}
					        	</div>
					        </div>
					        <div class="form-group">
					        	<div>
					        		{{ Form::textarea('descrption_'.$lang_->language->language, $serviceLang->description, ['class' => 'form-control', 'style'=>'height: 75px;','placeholder'=>trans('main.descrpcion en').' '.$lang_->language->language, 'autocomplete'=>'off']) }}
					        ...
					        @else
					            ...
					            <div class="form-group">
					            	<div>
					            		{{ Form::text($lang_->language->language, $serviceLang->name, ['class' => 'form-control', 'placeholder'=>$lang_->language->language, 'autocomplete'=>'off']) }}
                                        {{ errors_for($lang_->language->language, $errors) }}
					            	</div>
					            </div>
					            <div class="form-group">
