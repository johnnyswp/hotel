	...
	<div class="row">
		...
		<section class="panel" style="" >
			<header class="panel-heading sm" data-color="theme-inverse">
				<h2><strong>{{trans('main.Editar')}}</strong> {{trans('main.order')}}</h2>
       ...
       <p>{{trans('main.habitacion')}}: {{$stay->room->number_room}}</p>
        ...
        <div class="form-group" style="padding: 10px;">
            <div>
				...
				{{ Form::open(['files' => 'true', 'method' => 'POST', 'action'=>'ReceptionistController@anyOrderUpdate']) }}
                   {{ Form::hidden('order_id', $order->id) }}
                    <div class="form-group" style="width: 300px;">
                        ...
                        <div>
                            {{ Form::select('state',array('programmed'=> trans('main.programmed'),
                                    ...
                                    'finalized'=> trans('main.finalized')), $order->state, array('class' => 'form-control selectpicker', 'data-size'=>'10')) }}
                    ...
                    <div class="form-group"style="width: 300px;" >
                        ...
                        <div>
                            {{ Form::hidden('date', Carbon::parse($order->delivery_time), ['id' => 'date']) }}
                            {{ Form::text('fecha', Carbon::parse($order->delivery_time)->format('d-m-Y H:i')  , ['class' => 'form-control start-date', 'required'=>'required']) }}
        ...
        if(data.update==true){
          ...
          $('#total').html(parseInt($('#total').html())+parseInt(data.price));
            ...
            var element = '<tr id="item-order-'+data.id+'">'+
                          ...
                          '     </div>'+
                          '  </td>'+
                          '  <td>{{Hotel::find(Hotel::id())->exchanges->symbol}} '+data.price+'</td>'+
                          '  <td>{{Hotel::find(Hotel::id())->exchanges->symbol}} <span id="sub-total-'+data.id+'">'+data.price+'</span></td>'+
